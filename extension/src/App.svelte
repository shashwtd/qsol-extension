<script>
	...
	import ManageWallets from "./pages/ManageWallets.svelte";
	...
	onMount(async () => {
		...
		entities =
			(await browser.storage.local.get("cachedEntities")).cachedEntities ?? [];
		...
		let requestFunction = (await browser.storage.session.get("requestFunction"))
			.requestFunction;
